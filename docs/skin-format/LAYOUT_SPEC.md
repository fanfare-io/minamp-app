...
### Lessons learned (process, not coordinates)

- **Don't trust haiku-class agents for code-precision research.** The original spec was generated by a researcher that fabricated coordinates the Swift code never used (e.g. visualizer at full 275×42, EQ thumb at y=38). Read the source directly.
...
- **A 1px discrepancy between component dimension and renderer rect is normal**: e.g. Volume sprite is 68×15 per `SpriteExtractor.swift` but the slider rect is 68×14 per `MainWindowView.swift`. The sprite size wins for component dimensions.
